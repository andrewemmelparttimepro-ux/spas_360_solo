export type PartApplication={manufacturer:string;model:string;model_year_start:number|null;model_year_end:number|null;component:string;variant?:string|null};
const normalize=(value:string)=>value.toLowerCase().replace(/[^a-z0-9 ]/g,' ').replace(/\s+/g,' ').trim();
const brand=(value:string)=>normalize(value).replace(/\bspas?\b/g,'').trim();
export function exactPartApplication(row:PartApplication,request:{manufacturer:string;model:string;year:number;component:string}){
