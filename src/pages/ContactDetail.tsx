...
function EditableTypeBadge({ value, onSave }: { value: ContactType; onSave: (u: Partial<Contact>) => Promise<boolean> }) {
  ...
  if (editing) {
    return (
      <select
        ref={ref} value={value}
        onChange={async e => { await onSave({ customer_type: e.target.value as ContactType }); setEditing(false); }}
  ...
  return (
    <span
      ...
      className={cn("px-3 py-1 rounded-full text-sm font-medium cursor-pointer hover:ring-2 hover:ring-sky-200 transition-all group inline-flex items-center gap-1", TYPE_COLORS[value] ?? 'bg-slate-100 text-slate-800')}
...
function EditableDetailRow({ label, value, field, onSave, type = 'text' }: {
  label: string; value: string | null; field: string;
  onSave: (u: Partial<Contact>) => Promise<boolean>;
  type?: 'text' | 'email' | 'tel';
}) {
  const [editing, setEditing] = useState(false);
  ...
  const commit = async () => {
    ...
    setSaving(true);
    await onSave({ [field]: draft || null } as Partial<Contact>);
    setSaving(false);
    setEditing(false);
  };
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <dd
          onClick={() => setEditing(true)}
          className="text-sm text-slate-800 cursor-pointer rounded px-1.5 py-0.5 -mx-1.5 hover:bg-sky-50 hover:ring-1 hover:ring-sky-200 transition-colors group inline-flex items-center gap-1"
          title="Click to edit"
        >
          {value || '\u2014'}
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <EditableTypeBadge value={contact.customer_type} onSave={updateContact} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Contact Details</h2>
            <span className="text-[10px] text-slate-400">Click any value to edit</span>
          </div>
          <div className="space-y-4">
            <EditableDetailRow label="Phone" value={contact.phone} field="phone" onSave={updateContact} type="tel" />
            <EditableDetailRow label="Secondary Phone" value={contact.phone_secondary} field="phone_secondary" onSave={updateContact} type="tel" />
            <EditableDetailRow label="Email" value={contact.email} field="email" onSave={updateContact} type="email" />
            <EditableDetailRow label="Address" value={contact.mailing_address} field="mailing_address" onSave={updateContact} />
            <div>
              <dt className="text-xs font-medium text-slate-400 mb-0.5">Lead Source</dt>
              <dd className="text-sm text-slate-800">{contact.lead_source}</dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-400 mb-0.5">Tags</dt>
              <dd className="text-sm text-slate-800">{contact.tags?.join(', ') || '\u2014'}</dd>
            </div>
            <div>
              <dt className="text-xs font-medium text-slate-400 mb-0.5">Created</dt>
              <dd className="text-sm text-slate-800">{new Date(contact.created_at).toLocaleDateString()}</dd>
            </div>
          </div>
        </div>
