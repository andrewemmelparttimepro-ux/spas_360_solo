-- Save New Deal intake atomically, preserving normal table RLS for every write.
-- Rollback: drop function public.create_quick_deal(jsonb, text, date);
...
begin
  ...
  if coalesce(v_title, '') = '' or v_expected_close is null or p_next_activity_date is null then
    raise exception 'Deal title, expected close date and next activity date are required' using errcode = '22023';
  end if;
