alter table public.suggestions add column resolution_note text;
alter table public.suggestions add column resolution_release text;
alter table public.suggestions add column resolution_recorded_by uuid references public.profiles(id);
alter table public.suggestions add column resolution_recorded_at timestamptz;
grant update(resolution_note,resolution_release) on public.suggestions to authenticated;
create function private.stamp_suggestion_resolution() returns trigger language plpgsql security invoker set search_path='' as $$
begin
 if TG_OP='INSERT' then
  new.resolution_note:=null; new.resolution_release:=null;new.resolution_recorded_by:=null;new.resolution_recorded_at:=null;
 elsif new.resolution_note is distinct from old.resolution_note or new.resolution_release is distinct from old.resolution_release then
  if (select public.auth_role()) is distinct from 'owner_manager' then raise exception 'Owner access required' using errcode='42501';end if;
  new.resolution_recorded_by:=auth.uid();new.resolution_recorded_at:=now();
 end if;
 return new;
end $$;
create trigger stamp_suggestion_resolution before insert or update on public.suggestions for each row execute function private.stamp_suggestion_resolution();
