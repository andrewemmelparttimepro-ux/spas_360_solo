...
export interface OwnerAttentionSnapshot {
  ...
  scheduler: { name: string; active: boolean; last_status: string | null; last_finished: string | null; failures_7d: number }[];
}
...
export default function OwnerAttention() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(readError ? 'Owner overview could not refresh. Previously loaded facts remain dated below.' : null);
    if (!readError && data) setSnapshot(data as OwnerAttentionSnapshot);
    setLoading(false);
  ...
  return <section className="rounded-2xl border border-ink-700 bg-ink-900 p-5 space-y-4" aria-label="Owner Today">
    <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-ink-100">Owner Today</h2><p className="text-xs text-ink-500">{snapshot ? `Data read ${date(snapshot.as_of)}` : 'Loading the dealership picture'}</p></div><button disabled={loading} onClick={() => void refresh()} className="rounded-lg border border-ink-700 px-3 py-2 text-sm text-ink-100">{loading ? 'Refreshing…' : 'Refresh'}</button></div>
