...
function Coverage() {
  return <p className="rounded-lg border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
    Customer and job summaries are imported. Full notes, visits, files and billing history are pending the admin export. These records preserve Jobber history; daily scheduling remains in Schedule.
  </p>;
}
...
function HistoryList() {
  ...
  const kind = params.get('kind') === 'client' ? 'client' : 'job';
  ...
  return <div className="max-w-6xl mx-auto space-y-4 pb-8">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h1 className="text-2xl font-bold text-ink-100">Jobber History</h1><p className="mt-1 text-sm text-ink-400">Captured customer records and job summaries from both stores.</p></div>
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      <select aria-label="Record type" className={control} value={kind} onChange={e => update('kind', e.target.value)}><option value="job">Job summaries</option><option value="client">Customer records</option></select>
      ...
      <label className="relative flex-1 min-w-52"><Search className="absolute left-3 top-2.5 h-4 w-4 text-ink-500" /><input aria-label="Search Jobber history" placeholder="Name, job title, number or address" value={search} onChange={e => setSearch(e.target.value)} className={`${control} w-full pl-9`} /></label>
    </div>
    ...
    <div aria-live="polite" className="text-sm text-ink-400">{busy ? 'Loading history…' : `${count.toLocaleString()} ${kind === 'job' ? 'job summaries' : 'customer records'}${review ? ' needing a customer match' : ''}`}</div>
    ...
    <div className={`space-y-2 ${busy ? 'opacity-50' : ''}`}>
      {rows.map(row => <Link key={row.id} to={`/jobber-history/${row.id}`} className="block rounded-xl border border-ink-700 bg-ink-900 p-4 hover:border-brand-500">
        ...
        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink-500">{row.occurred_at && <span>Job start: {date(row.occurred_at)} CT</span>}<span className={row.match_status === 'review' ? 'text-amber-300' : 'text-emerald-400'}>{row.contact_id ? 'Linked to SPAS customer' : 'Customer match needs review'}</span></div>
      </Link>)}
    </div>
...
function HistoryDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const result = await supabase.from('jobber_history').select(`${FIELDS},candidate_contact_ids`).eq('org_id', profile.org_id).eq('id', id).maybeSingle();
      if (!current) return;
  ...
  return <div className="max-w-4xl mx-auto space-y-5 pb-8">
    ...
    <div><p className="text-sm text-ink-400">{row.source_account_name} · {customer ? 'Customer record' : `Job #${row.source_number}`}</p><h1 className="mt-1 text-2xl font-bold text-ink-100">{row.title}</h1></div>
    <Coverage />
    <section className="rounded-xl border border-ink-700 bg-ink-900 p-5 space-y-3">
      ...
      {row.contact_id ? <Link className="text-brand-300 hover:underline" to={`/customers/${row.contact_id}`}>Open SPAS customer</Link> : <p className="text-amber-300">Customer match needs review. This record is preserved here while the match is confirmed.</p>}
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 text-sm">
        ...
        {!customer && <div><dt className="text-ink-500">Job type</dt><dd className="mt-1">{row.summary.job_type === 'ONE_OFF' ? 'One-off' : row.summary.job_type === 'RECURRING' ? 'Recurring' : 'Not captured'}</dd></div>}
        ...
        {customer ? <>
          ...
          <div><dt className="text-ink-500">Phone</dt><dd className="mt-1">Pending detailed export</dd></div>
          ...
          <div><dt className="text-ink-500">Job start (Central time)</dt><dd className="mt-1">{date(row.summary.start_at)}</dd></div>
          ...
          <div><dt className="text-ink-500">Jobber job total</dt><dd className="mt-1">{row.summary.total == null ? 'Not captured' : new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(row.summary.total)}<p className="mt-1 text-xs text-ink-500">Job total; balance due has not been reconciled.</p></dd></div>
        </>}
      </dl>
    </section>
