...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!TWILIO_SID || !TWILIO_TOKEN || !TWILIO_FROM) {
    return res.status(500).json({ error: 'Twilio is not configured. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM.' });
  }
  ...
  if (!e164) return res.status(400).json({ error: `Can't parse phone number "${to}"` });
  ...
  if (!twilioRes.ok) {
    return res.status(twilioRes.status).json({ error: data?.message ?? 'Twilio send failed' });
  }
