-- Let a salesperson attach an available inventory unit while the deal is still
-- active. The final Closed-Won transaction remains responsible for marking the
-- unit Sold, assigning the customer, and triggering the Delivery handoff.
...
declare
  v_is_won boolean;
begin
  select coalesce(s.is_won, false)
  into v_is_won
  from public.pipeline_stages s
  where s.id = new.stage_id and s.org_id = new.org_id;

  if not found then
    raise exception 'Deal stage not found';
  end if;
  ...
  if v_is_won then
    ...
    where i.id = new.inventory_item_id
      and i.org_id = new.org_id
      and (
        (i.status = 'In Stock' and i.customer_id is null and i.deal_id is null)
        or (i.status = 'Sold' and i.customer_id = new.contact_id and i.deal_id = new.id)
      )
  ) then
...
declare
  ...
  v_current_stage_is_won boolean;
...
begin
  ...
  select coalesce(s.is_won, false)
  into v_current_stage_is_won
  from public.pipeline_stages s
  where s.id = v_current_stage_id and s.org_id = v_org;
  ...
  -- A preselection may change while the deal is active. Once Closed-Won, the
  -- completed choice is immutable and only an exact idempotent repeat is safe.
  if v_current_stage_is_won and v_existing_type is not null and (
  ...
  elsif exists (select 1 from public.inventory_items i where i.deal_id = p_deal_id) then
