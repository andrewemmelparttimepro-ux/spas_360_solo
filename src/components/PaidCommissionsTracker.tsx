...
import { CalendarDays, FileSpreadsheet, LoaderCircle, Pencil, Plus, Trash2, X } from 'lucide-react';
...
function currentPaidPeriod(): PaidCommissionDateRange {
...
export function PaidCommissionsTracker() {
  ...
  const [range, setRange] = useState(currentPaidPeriod);
  const [activeSalesperson, setActiveSalesperson] = useState<PaidCommissionSalesperson>(PAID_COMMISSION_SALESPEOPLE[0]);
  const [editor, setEditor] = useState<EditorState | null>(null);
  const rangeIsValid = paidCommissionDateRangeValid(range);
  const commissions = usePaidCommissions(range.startDate, range.endDate, isOpen && rangeIsValid);
  const entries = useMemo(
    () => commissions.entries.filter(entry => entry.salesperson_name === activeSalesperson),
    [activeSalesperson, commissions.entries],
  );
  ...
  const changeRange = (field: keyof PaidCommissionDateRange, value: string) => {
    setRange(current => ({ ...current, [field]: value }));
    setEditor(null);
  };

  return (
    <section aria-labelledby="paid-commissions-heading">
      <button
        type="button"
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span id="paid-commissions-heading" className="block text-base font-bold text-ink-100 group-hover:text-amber-600">Paid Commissions</span>
          <span className="mt-1 block text-sm text-ink-500">Open one commission workbook with a sheet for each salesperson.</span>
        </span>
        <span className="rounded-lg border border-ink-700 px-3 py-2 text-xs font-bold text-ink-300 group-hover:border-amber-500/60 group-hover:text-amber-600">Open workbook</span>
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/70 p-2 backdrop-blur-sm sm:p-5" onClick={closeWorkbook}>
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="paid-commissions-workbook-title"
          ...
          >
            <DialogKeys onClose={closeWorkbook} />
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="min-h-0 flex-1 overflow-auto p-3 sm:p-5">
                ...
                {!rangeIsValid ? (
                  ...
                  <SalespersonCommissionSheet
                    salesperson={activeSalesperson}
                    entries={entries}
                    editor={editor?.salesperson === activeSalesperson ? editor : null}
                    paidPeriod={range}
                    onAdd={() => setEditor({ salesperson: activeSalesperson, entry: null })}
                    onEdit={entry => setEditor({ salesperson: activeSalesperson, entry })}
                    onCancel={() => setEditor(null)}
                    onSave={async values => {
                      const saved = await commissions.saveEntry({ ...values, salespersonName: activeSalesperson }, editor?.entry?.id);
              ...
              <div className="overflow-x-auto border-t border-ink-700 bg-ink-950 px-2 pt-2" role="tablist" aria-label="Paid commission salesperson sheets">
                <div className="flex min-w-max items-end gap-1">
                  {PAID_COMMISSION_SALESPEOPLE.map(salesperson => (
                    <button
                      ...
                      type="button"
                      role="tab"
                      aria-selected={activeSalesperson === salesperson}
                      onClick={() => { setActiveSalesperson(salesperson); setEditor(null); }}
                      className={`min-w-24 rounded-t-lg border px-4 py-2.5 text-sm font-bold ${activeSalesperson === salesperson ? 'border-amber-500/60 border-b-ink-900 bg-ink-900 text-amber-500' : 'border-ink-700 bg-ink-950 text-ink-500 hover:text-ink-200'}`}
                    >
                      {salesperson}
                    </button>
                  ))}
...
function SalespersonCommissionSheet({ salesperson, entries, editor, paidPeriod, onAdd, onEdit, onCancel, onSave, onDelete }: {
  salesperson: PaidCommissionSalesperson;
  entries: PaidCommission[];
  editor: EditorState | null;
  paidPeriod: PaidCommissionDateRange;
  onAdd: () => void;
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-ink-700 bg-ink-950/50">
      <header className="flex flex-wrap items-center justify-between gap-3 bg-ink-950 px-4 py-3">
        <div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-sm">
          ...
          <tfoot><tr className="border-t border-ink-700 bg-ink-950 font-bold text-ink-100"><td colSpan={4} className="px-3 py-3 text-right">{salesperson} total</td><td className="px-3 py-3 text-right">{currency.format(total)}</td><td /></tr></tfoot>
        </table>
      </div>
      {editor && <CommissionEditor key={editor.entry?.id ?? `${salesperson}-new`} entry={editor.entry} paidPeriod={paidPeriod} onCancel={onCancel} onSave={onSave} />}
    </article>
  );
}

function CommissionEditor({ entry, paidPeriod, onCancel, onSave }: { entry: PaidCommission | null; paidPeriod: PaidCommissionDateRange; onCancel: () => void; onSave: (values: PaidCommissionValues) => Promise<boolean> }) {
  const [paidOn, setPaidOn] = useState(entry?.paid_on ?? paidPeriod.endDate);
  ...
  const values = { paidOn, customerName, saleAmount: Number(saleAmount), commissionPercentage: Number(commissionPercentage) };
  const paidOnInPeriod = paidOn >= paidPeriod.startDate && paidOn <= paidPeriod.endDate;
