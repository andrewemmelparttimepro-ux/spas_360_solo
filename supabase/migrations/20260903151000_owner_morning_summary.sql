-- Owner Morning Summary: one owner-only read of the prior day across staff
-- attendance, delegated tasks, deals, service, customers, texts, suggestions.
...
declare
  ...
  v_misc jsonb;
begin
  if auth.uid() is null or (select public.auth_role()) <> 'owner_manager' then
  ...
  from (
    select p.id,
           ...
           coalesce((
             select jsonb_agg(jsonb_build_object('title', left(t.title, 140), 'due_at', t.due_at, 'overdue', t.due_at is not null and t.due_at < now()) order by t.due_at nulls last, t.created_at)
             from public.tasks t
             where t.assigned_to = p.id and t.task_type = 'Delegated' and t.status <> 'Completed'
  ...
  select jsonb_build_object(
    ...
    'won', coalesce((
      ...
      where d.org_id = v_org and s.is_won and coalesce(d.closed_at, d.updated_at) >= v_start and coalesce(d.closed_at, d.updated_at) < v_end
    ), '[]'::jsonb),
    'lost', coalesce((
      ...
      where d.org_id = v_org and s.is_lost and coalesce(d.closed_at, d.updated_at) >= v_start and coalesce(d.closed_at, d.updated_at) < v_end
    ), '[]'::jsonb),
    'stage_changes', (
  ...
  select jsonb_build_object(
    ...
    'scheduled_today', coalesce((
      ...
      from public.jobs j where j.org_id = v_org and j.scheduled_at >= v_today_start and j.scheduled_at < v_today_end and j.status <> 'Completed'
    ), '[]'::jsonb)
  ) into v_jobs;
  ...
  select jsonb_build_object(
    ...
    'open', (select count(*) from public.tasks t where t.org_id = v_org and t.task_type = 'Delegated' and t.status <> 'Completed')::int,
    'overdue', (select count(*) from public.tasks t where t.org_id = v_org and t.task_type = 'Delegated' and t.status <> 'Completed' and t.due_at < now())::int
  ) into v_delegated;
  ...
  select jsonb_build_object(
    ...
    'inbound_texts', (
      select count(*) from public.messages m join public.communication_threads th on th.id = m.thread_id
      where th.org_id = v_org and th.thread_type = 'sms' and m.sender_type = 'customer' and m.created_at >= v_start and m.created_at < v_end
    )::int,
...
-- 7:30 AM Central heads-up for every owner: one line of numbers and a link.
create or replace function public.send_morning_summary_notice()
returns void
language plpgsql
security definer
set search_path = ''
...
declare
  v_org record;
  v_owner record;
  v_day date := ((now() at time zone 'America/Chicago')::date - 1);
  v_start timestamptz := (v_day::timestamp) at time zone 'America/Chicago';
  v_end timestamptz := ((v_day + 1)::timestamp) at time zone 'America/Chicago';
  v_clocked int;
  v_done int;
...
begin
  for v_org in select id from public.organizations loop
    select count(distinct e.user_id) into v_clocked from public.staff_time_entries e
      where e.org_id = v_org.id and e.clock_in >= v_start and e.clock_in < v_end;
    select count(*) into v_done from public.tasks t
      where t.org_id = v_org.id and t.task_type = 'Delegated' and t.status = 'Completed' and t.completed_at >= v_start and t.completed_at < v_end;
    select count(*) into v_open from public.tasks t
      where t.org_id = v_org.id and t.task_type = 'Delegated' and t.status <> 'Completed';
    select count(*) into v_won from public.deals d join public.pipeline_stages s on s.id = d.stage_id
      where d.org_id = v_org.id and s.is_won and coalesce(d.closed_at, d.updated_at) >= v_start and coalesce(d.closed_at, d.updated_at) < v_end;
    select count(*) into v_flags from public.staff_time_entries e
      where e.org_id = v_org.id and e.clock_out >= v_start and e.clock_out < v_end and e.acknowledged_incomplete_count > 0;

    v_title := 'Morning Summary — ' || to_char(v_day, 'Dy Mon DD');
    for v_owner in
      select id from public.profiles
      where org_id = v_org.id and role = 'owner_manager' and lower(coalesce(email, '')) <> 'thrawn@ndai.pro'
    loop
      if not exists (
        select 1 from public.notifications n
        where n.user_id = v_owner.id and n.type = 'morning_summary' and n.title = v_title
      ) then
        ...
        values (
          ...
          'morning_summary',
          v_title,
          v_clocked || ' clocked in · ' || v_done || ' tasks done · ' || v_open || ' still open · '
            || v_won || ' deal' || case when v_won = 1 then '' else 's' end || ' won'
            || case when v_flags > 0 then ' · ' || v_flags || ' clocked out with open tasks' else '' end,
...
revoke all on function public.send_morning_summary_notice() from public, anon, authenticated;

do $$
begin
  perform cron.unschedule('spas360-morning-summary');
exception when others then
  null;
end $$;
select cron.schedule('spas360-morning-summary', '30 12 * * *', 'select public.send_morning_summary_notice()');
