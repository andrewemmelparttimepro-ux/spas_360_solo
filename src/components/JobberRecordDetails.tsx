...
export function JobberRecordDetails({ raw, title }: { raw: unknown; title: string }) {
  ...
  return <>
    ...
    {text(record.__typename) === 'JobberCommunication' && <section className={section}><h2 className="font-semibold">Communication</h2><dl className="grid gap-3 sm:grid-cols-2">{Object.entries(object(record.report)).filter(([key, value]) => key !== 'Open' && Boolean(text(value)) && text(value) !== '-').map(([key, value]) => <div key={key}><dt className="text-xs text-ink-400">{key === '~Sent time' ? 'Sent time (Central)' : key}</dt><dd className="mt-1 whitespace-pre-wrap break-words text-sm">{text(value)}</dd></div>)}</dl></section>}
