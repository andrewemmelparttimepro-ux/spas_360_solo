...
function EditableStatusBadge({ value, onSave }: { value: JobStatus; onSave: (u: Partial<Job>) => Promise<boolean> }) {
  ...
  return (
    <span
      onClick={() => setEditing(true)}
      className={cn("px-3 py-1 rounded-lg text-sm font-bold border-l-4 cursor-pointer hover:ring-2 hover:ring-sky-200 transition-all group inline-flex items-center gap-1", statusColors[value] ?? 'bg-slate-100')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <EditableField label="Collect" value={job.amount_to_collect} field="amount_to_collect" onSave={saveJob} icon={DollarSign} type="number" prefix="$" bold color="text-emerald-600" />
          </div>
        </div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <EditableStatusBadge value={job.status as JobStatus} onSave={updateJob} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-5">
          ...
          <div className="space-y-4">
            <EditableField label="Job Type" value={job.job_type} field="job_type" onSave={updateJob} icon={Wrench} type="select" options={JOB_TYPE_OPTIONS} />
            {location && <div className="flex items-center text-sm"><MapPin className="w-4 h-4 mr-2 text-slate-400" />{location.name}</div>}
            {property && <EditableField label="Address" value={property.address} field="address" onSave={updateJob} icon={MapPin} />}
            <EditableField label="Scheduled" value={job.scheduled_at ? new Date(job.scheduled_at).toISOString().slice(0, 16) : null} field="scheduled_at" onSave={updateJob} icon={Clock} type="datetime-local" />
            {job.estimated_duration && <div className="text-sm text-slate-600">Duration: {job.estimated_duration} min</div>}
            {job.description && <div className="text-sm text-slate-600 bg-slate-50 p-3 rounded-lg">{job.description}</div>}
            <EditableField label="Collect" value={job.amount_to_collect} field="amount_to_collect" onSave={updateJob} icon={DollarSign} type="number" prefix="$" bold color="text-emerald-600" />
          </div>
        </div>
