    ...
    <div className="h-full flex flex-col max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-1.5 mb-4 shrink-0">
        ...
        {jobTypeFilter.size > 0 && (
          <button type="button" onClick={() => setJobTypeFilter(new Set())} className="text-[11px] font-semibold text-brand-400 hover:text-brand-300 px-2">
            Clear filter
          </button>
        )}
      </div>
              ...
              <div className="rounded-xl border border-ink-700 bg-ink-950 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-ink-300">Current Inventory <span className="font-normal text-ink-500">(optional)</span></p>
                    <p className="mt-1 truncate text-xs text-ink-500">
                      {selectedInventory ? inventoryUnitLabel(selectedInventory) : 'No inventory unit attached'}
                    </p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    {selectedInventory && (
                      <button type="button" onClick={() => setNewJob({ ...newJob, inventory_item_id: '' })} className="rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink-400 hover:bg-ink-800 hover:text-ink-200">
                        Remove
                      </button>
                    )}
                    <button
                      ...
                      disabled={!newJob.location_id || inventoryLoading || availableInventory.length === 0}
                      className="rounded-lg border border-brand-500/40 bg-brand-500/10 px-3 py-1.5 text-xs font-semibold text-brand-300 hover:bg-brand-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {selectedInventory ? 'Change unit…' : 'Choose unit…'}
                    </button>
                  </div>
                </div>
                {!newJob.location_id && <p className="mt-2 text-xs text-amber-300">Choose a location to see available stock.</p>}
                {newJob.location_id && !inventoryLoading && availableInventory.length === 0 && <p className="mt-2 text-xs text-ink-500">No unassigned In Stock units are available at this location.</p>}
              </div>
      ...
      {showCreate && showInventorySelector && (
        <DealInventorySelector
          ...
          busy={creatingJob}
          showStore={locations.length > 1}
          title="Attach inventory to this job"
          actionLabel="Attach unit"
          onCancel={() => setShowInventorySelector(false)}
          onConfirm={inventoryItemId => {
            setNewJob(current => ({ ...current, inventory_item_id: inventoryItemId }));
            setShowInventorySelector(false);
          }}
        />
      )}
