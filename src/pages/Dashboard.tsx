...
const statMeta = [
  ...
  { key: 'activeDeals', title: 'Active Deals', icon: Users, color: 'text-blue-600', bg: 'bg-blue-100', format: (v: number) => String(v), link: '/crm' },
  { key: 'unscheduledJobs', title: 'Unscheduled Jobs', icon: Wrench, color: 'text-amber-600', bg: 'bg-amber-100', format: (v: number) => String(v), link: '/service' },
  { key: 'overduePartsCount', title: 'Parts Overdue', icon: AlertCircle, color: 'text-red-600', bg: 'bg-red-100', format: (v: number) => String(v), link: '/inventory' },
] as const;
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statMeta.map((meta) => {
          ...
          return (
            <Link key={meta.key} to={meta.link} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex items-start justify-between hover:shadow-md hover:border-sky-200 transition-all">
              <div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <div className="space-y-4">
            {actions.length === 0 ? (
              ...
              <Link key={action.id} to={actionLinks[action.type] ?? '/service'} className="flex items-start p-3 hover:bg-sky-50 rounded-lg transition-colors cursor-pointer border border-transparent hover:border-sky-100 group">
                <div className={`w-2 h-2 mt-2 rounded-full flex-shrink-0 ${actionDotColors[action.type]}`} />
                <div className="ml-3 flex-1">
                  <p className="text-sm font-medium text-slate-900 group-hover:text-sky-700">{action.title}</p>
